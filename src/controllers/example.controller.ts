...
export const createExample = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Basic validation
    if (!exampleData.name || !exampleData.description || !exampleData.price || !exampleData.metadata?.category) {
      res.status(400).json({
        success: false,
        message: 'Missing required fields: name, description, price, and metadata.category are required'
      });
      return;
    }
...
export const getExamples = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await exampleService.getExamples(page, limit, category, isDeleted);
 ...
 */
export const getExamplesByCategory = async (req: Request, res: Response): Promise<void> => {
  try {
 ...
 */
export const searchExamples = async (req: Request, res: Response): Promise<void> => {
  try {
    const { q } = req.query;
